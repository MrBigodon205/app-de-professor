...
export const BulkTransferModal: React.FC<BulkTransferModalProps> = ({ isOpen, onClose, studentIds, onSuccess }) => {
    const { classes, bulkTransferStudents } = useClass();
    ...
    const handleTransfer = async () => {
        if (!targetSeriesId || !targetSection || studentIds.length === 0) return;

        setLoading(true);
        try {
            const success = await bulkTransferStudents(studentIds, targetSeriesId, targetSection);
            if (success) {
                onSuccess?.();
                onClose();
            }
        } catch (error) {
            alert('Erro ao transferir alunos. Tente novamente.');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="bg-white dark:bg-slate-950 w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden relative z-10 animate-in zoom-in-95 duration-300 border border-slate-200 dark:border-slate-800">
                ...
                <div className={`p-8 bg-gradient-to-br from-${theme.primaryColor} to-${theme.secondaryColor} text-white relative`}>
                    ...
                    <p className="text-white/80 font-medium text-sm">
                        Movendo <span className="text-white font-black">{studentIds.length} alunos</span> para uma nova turma.
                    </p>
                </div>
                    ...
                    <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/20 flex gap-3 items-start">
                        ...
                        <div>
                            <p className="text-amber-800 dark:text-amber-400 text-xs font-bold leading-relaxed">
                                Movendo múltiplos alunos
                            </p>
                            <p className="text-amber-700/70 dark:text-amber-400/70 text-[10px] font-medium leading-relaxed mt-1">
                                Todos os registros (frequência, notas, observações) serão preservados na nova turma.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-8 pt-0 flex gap-3">
                    ...
                    <button
                        ...
                        disabled={loading || !targetSeriesId || !targetSection || studentIds.length === 0}
