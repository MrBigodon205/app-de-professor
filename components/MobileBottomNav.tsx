...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({ onMoreClick }) => {
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 h-[72px] bg-white dark:bg-surface-dark border-t border-slate-200 dark:border-slate-800 flex items-center justify-around px-2 z-50 pb-safe md:hidden shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            {navItems.map((item) => (
                <NavLink
                    key={item.path}
                    to={item.path}
