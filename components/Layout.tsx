...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="fixed inset-0 w-full h-[100dvh] flex bg-transparent overflow-hidden selection:bg-primary/10 selection:text-primary">
      ...
      <aside
        className={`fixed top-0 left-0 z-50 h-[100dvh] transition-[width,border-color] duration-200 ease-out flex flex-col overflow-hidden
      ...
      {/* NEW: External Floating Sidebar Toggle */}
      <motion.button
        onClick={toggleSidebar}
        initial={false}
        animate={{
          left: isSidebarCollapsed ? "1rem" : "19.5rem", // 1rem (16px) vs 19.5rem (312px). Sidebar is 18rem (288px). 
          rotate: isSidebarCollapsed ? 0 : 180
        }}
        transition={{ duration: 0.2, ease: "easeOut" }} // Sync with sidebar transition
        className="hidden lg:flex fixed top-8 z-[60] items-center justify-center size-12 rounded-full bg-surface-elevated border border-border-default text-text-muted hover:text-primary shadow-lg hover:shadow-xl hover:shadow-primary/20 backdrop-blur-xl transition-colors active:scale-95 origin-center"
      ...
      {/* Main Content Area */}
      <div
        className={`flex-1 flex flex-col min-w-0 h-full relative z-10 transition-[margin] duration-300 ease-[cubic-bezier(0.25,0.1,0.25,1.0)]
      ...
      >
        ...
        <header
          className={`flex flex-col md:flex-row items-center justify-between mx-2 md:mx-4 mt-2 md:mt-4 mb-2 rounded-2xl px-2 py-1.5 md:px-3 md:py-3 z-[40] shrink-0 gap-3 sticky top-2 min-h-[48px] md:min-h-[60px] transition-all duration-300
            bg-white/90 dark:bg-slate-900/60 backdrop-blur-md
        ...
        >
          ...
          <div className="flex w-full items-center justify-between gap-x-2 gap-y-0">
            ...
            {/* 1. LEFT: Menu & Context */}
            <div className={`flex items-center gap-3 shrink-0 lg:w-auto relative z-50 transition-all duration-300 lg:pl-14`}>
              <button
              ...
              >
                <div className={`size-8 md:size-9 rounded-lg flex items-center justify-center shadow-sm shrink-0 overflow-visible transition-transform group-hover:scale-105 animate-neon-pulse-box ${isInstitutionalRoute ? 'bg-white border border-border-default' : 'bg-transparent'}`}>
                  {isInstitutionalRoute ? (
                    currentSchool?.logo_url ? (
                      ...
                      <span className="material-symbols-outlined text-primary text-xl animate-neon-pulse-logo">school</span>
                    )
                  ) : (
                ...
                <div className="flex flex-col items-start leading-tight max-w-[100px] md:max-w-xs">
                  <span className="text-[9px] md:text-[10px] font-bold text-text-muted uppercase tracking-wider truncate w-full text-left group-hover:text-primary transition-colors">
                ...
                <span className="material-symbols-outlined text-text-muted text-xs group-hover:text-primary transition-colors">expand_more</span>
              </button>
            </div>

            {/* 2. CENTER: Series + Classes */}
            {!isCoordinator && (
            ...
            <div className="flex items-center justify-end gap-4 md:gap-6 shrink-0 ml-auto bg-transparent md:bg-surface-subtle/40 p-0 md:p-2 rounded-2xl order-2 md:order-3">
              {!isCoordinator && (
                <button
                  onClick={() => setIsClassSelectorOpen(true)}
                  className="lg:hidden flex items-center gap-2 bg-white dark:bg-slate-800/80 px-3 h-10 rounded-xl border border-border-default active:scale-95 transition-all shadow-sm group hover:border-primary/40"
                >
                  ...
                  <span className="material-symbols-outlined text-[16px] text-text-muted transition-transform group-hover:rotate-180">expand_more</span>
                </button>
              )}
                ...
                <div className="relative shrink-0 hidden md:block">
                  {isCoordinator ? (
                    ...
                    <>
                      {availableSubjects.length > 0 && (
                        <>
                          <button onClick={() => setIsSubjectDropdownOpen(!isSubjectDropdownOpen)} className="flex items-center gap-3 px-4 h-10 rounded-xl text-sm font-bold text-text-secondary hover:bg-surface-subtle hover:text-primary transition-all border border-border-default bg-surface-card shadow-sm overflow-hidden group">
                            <span className={`size-2.5 rounded-full bg-primary shadow-neon shrink-0 animate-pulse`}></span>
                            <span className="inline-block max-w-[150px] lg:max-w-[200px] truncate">{activeSubject || 'Selecione...'}</span>
